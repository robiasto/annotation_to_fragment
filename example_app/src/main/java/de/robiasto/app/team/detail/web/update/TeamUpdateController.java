...
@Controller
@RequestMapping(TeamId.ROUTE_BASE_PATH + RouteConfiguration.UPDATE)
@AllArgsConstructor
class TeamUpdateController extends AbstractValidationController<TeamResponse> {
    ...
    public ModelAndView displayForm(@PathVariable("id") TeamEntity team) {
        this.formView.initialize(
                ...
                new TeamFormView(
                        team,
                        updateService.getCoaches(team.getId()),
                        updateService.getPlayers(team.getId())
                ),
    ...
    public ModelAndView edit(
        ...
        if (!bindingResult.hasErrors()) {
            updateService.update(team, formData);
